...
Zerovibe — от первой фичи до прод-деплоя на `<поддомен>.zerovibe.ru` с авто-TLS.
Плагин даёт готовый шаблон проекта, помогает добавлять функциональность и
деплоит за тебя — DevOps не нужен.
