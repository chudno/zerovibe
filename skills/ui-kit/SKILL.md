...
выверенные дизайн-принципы (адаптировано из github.com/pbakaus/impeccable,
Apache-2.0). **Это НЕ опционально: перед вёрсткой/редизайном прочитай нужный файл.**

- **`impeccable/product.md`** — ВСЕГДА читай первым. Режим продуктового UI
  (дашборды, формы, таблицы, инструменты): консистентность, плотность, состояния,
  что выдаёт слоп. Наши приложения — этот режим.
- **`impeccable/layout.md`** — отступы, ритм, иерархия, ширина, сетка. Читай, когда
  страница «расползается» или выглядит плоско.
- **`impeccable/typeset.md`** — типографика: шкала, вес, длина строки.
- **`impeccable/colorize.md`** — цвет, контраст, палитра (в т.ч. цвета графиков
  в тон темы, а не дефолтный синий).
- **`impeccable/craft.md`** — общая планка качества и частые дефекты.
- **`impeccable/critique.md`** — чек-лист самопроверки: пройди перед сдачей
  («если кто-то скажет *это сделал ИИ* — переделай»).
