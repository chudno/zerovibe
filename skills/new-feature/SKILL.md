...
- Схема таблицы — НЕ в коде, а НОВОЙ goose-миграцией:
  `internal/platform/db/migrations/NNNNN_<x>s.sql` (следующий номер) с секциями
  `-- +goose Up` (`CREATE TABLE ...`) и `-- +goose Down` (`DROP TABLE ...`).
  Миграции применяются автоматически при старте (`db.MigrateUp`) — ничего вызывать
  руками не нужно.
