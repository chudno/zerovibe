...
- **ПЕРЕД вёрсткой шаблонов ОТКРОЙ skill `ui-kit` и прочитай все 6 файлов
  `impeccable/` (product, layout, typeset, colorize, craft, critique).** Без этого
  страница выйдет «как сгенерированная ИИ»: плоские отступы, всё на весь экран. Это
  не опция — без чтения `layout.md` отступы будут плохими. Перед сдачей пройди
