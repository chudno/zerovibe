...
6. **Подведи итог и подскажи, что дальше** (вайбкодер не должен лезть в доки —
   всё нужное скажи здесь). Сообщи примерно так:

   > Готово! Проект «<имя>» создан, папка привязана.
   > Дальше просто говори мне обычными словами, что нужно в приложении —
   > например «добавь раздел задач» или «сделай форму входа». Я напишу код.
   > Когда захочешь опубликовать — скажи «задеплой» (или `/zerovibe:deploy`),
   > и приложение появится по адресу `<поддомен>.zerovibe.ru`.

   Никаких других команд учить не нужно — только эта памятка.
