...
- **Платформенные переменные прокидываются автоматически.** При деплое платформа сама
  кладёт в контейнер `PLATFORM_API_URL`/`PLATFORM_API_KEY` (по ним приложение шлёт письма
  через платформу — нужно для восстановления пароля), `APP_BASE_URL` (свой домен) и
  `SECURE_COOKIE`. Креатор о них не думает, задавать руками не нужно.
- Схема БД доезжает сама: миграции goose применяются при старте контейнера
  (`db.MigrateUp`). Новые таблицы из новых миграций создаются автоматически.
